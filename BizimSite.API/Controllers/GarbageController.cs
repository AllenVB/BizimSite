...
public class GarbageController : ControllerBase
{
    ...
    public GarbageController(AppDbContext db) => _db = db;

    [HttpGet]
    public async Task<IActionResult> GetAll()
    {
        var list = await _db.GarbageTrackings.Include(g => g.User).Where(g => g.IsReady)
            .Select(g => new GarbageResponse(g.Id, g.User != null ? g.User.Name : "", g.Block, g.No, g.IsReady, g.MarkedAt, g.CollectedAt)).ToListAsync();
        return Ok(list);
    }
    ...
    public async Task<IActionResult> Mark()
    {
        ...
        _db.GarbageTrackings.Add(new GarbageTracking { UserId = userId, Block = user.Block, No = user.No });
